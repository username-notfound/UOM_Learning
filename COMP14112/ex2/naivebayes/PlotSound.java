...
public class PlotSound {
	...
	String wavDataDirectory = "data/yesno/wav/";
	...
	ampGraph.setLimits(0.0,lengthAmpSequence/(double) DataWithWav.wavRate+0.01,lower_limit,upper_limit);
	ampGraph.setColor(2);
	ampGraph.showGraph();
	...
	mfccGraph.showGraph();
	mfccGraph.setTitle("MFCC 1 over time");
	...
	// Plot the 1st MFCC for the same example
	...
	// Plot the time-averaged 1st MFCC on the same plot in a different colour

	mfccGraph.nextGraph();
	mfccGraph.setColor(1);
	for (int i=0; i<lengthMfccSequence; i++) {

	    double time = i/(double) Data.mfccRate; // Work out the time
	    mfccGraph.add(time,exampleData.getMeanMfcc(exampleNumber)[0]); // Add the first MFCC component at this time
	}
	mfccGraph.showGraph();
	mfccGraph.setTitle("Time-averaged MFCC 1");
    }
}
